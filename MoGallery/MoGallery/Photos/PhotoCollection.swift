...
class PhotoCollection: NSObject, ObservableObject {
    ...
    unowned let photosModel: PhotosModel
    ...
    init(_ photosModel: PhotosModel, albumNamed albumName: String, createIfNotFound: Bool = false) {
    ...
    init(_ photosModel: PhotosModel, smartAlbum smartAlbumType: PHAssetCollectionSubtype) {
        self.photosModel = photosModel
    ...
    func photoLibraryDidChange(_ changeInstance: PHChange) {
        Task { @MainActor in
            ...
            guard let changes = changeInstance.changeDetails(
                for: self.photoAssets.fetchResult) else { return }
            await self.refreshPhotoAssets(changes.fetchResultAfterChanges)
        }
    }
}

//fileprivate let logger = Logger(subsystem: "com.jht1900.CaptureCameraStorage", category: "PhotoCollection")
